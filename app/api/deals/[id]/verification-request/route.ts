    ...
    if (RESEND_API_KEY) {
      ...
      await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          to: [inspector_email],
          subject: `Inspection request: ${deal.commodity_type} · ${deal.volume_tonnes}t · Ref: ${dealId.slice(0, 8)}`,
          html: `
            <div style="font-family:-apple-system,sans-serif;max-width:480px;margin:0 auto;background:#0f172a;color:#e2e8f0;padding:32px;border-radius:12px;">
              ...
              <p style="color:#94a3b8;font-size:13px;">${profile?.company_name || 'A trader'} has requested a ${inspector_type.replace(/_/g, ' ')} for a ${deal.commodity_type} deal (${deal.volume_tonnes.toLocaleString()}t).</p>
              <div style="background:#1e293b;border-radius:8px;padding:16px;margin:16px 0;">
                <p style="font-size:13px;color:#e2e8f0;">Type: ${inspector_type.replace(/_/g, ' ')}</p>
                <p style="font-size:13px;color:#e2e8f0;">Commodity: ${deal.commodity_type}</p>
                ...
                ${notes ? `<p style="font-size:13px;color:#94a3b8;">Notes: ${notes}</p>` : ''}
              </div>
